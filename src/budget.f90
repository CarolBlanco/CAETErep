...
contains
   ...
   subroutine daily_budget(dt, w1, w2, ts, temp, prec, n_days, p0, ipar, rh&
        ...
        &, delta_cveg_1, seed_bank_out_1, co2_abs_se_1, limitation_status_1, uptk_strat_1, cp, c_cost_cwm)
...
!     ----------------------------INPUTS-------------------------------
      ...
      real(r_8),dimension(npls),intent(out) ::  seed_bank_out_1 !! NEW (module_reproduction) 
      ...
      do p = 1,nlen
         ...
         seed_bank_int(ri) = seed_bank_out_1(p)
      ...
      do p = 1, nlen
         ...
         seed_bank_out_1(ri)= seed_bank_int(p) !! NEW (module_reproduction)
